...
- 2026-09-17 — general — consistency — Project improvement review (mandatory memory-layer consistency check, per `prompts/project-improvement.md`)
  The 2026-09-15 "Resolved" entry below (write-back instructions tightened to "same turn") explicitly flagged itself as unverified pending a future account's actual glossary section. Checked now: `logs/history.jsonl` shows real, substantial Requirements Intake work (recommendations, charts, dashboards, drill-downs) for accounts 38992, 44663, 62956, 96467, and 101731 — none of them has a `## Account <n>` section in `references/metric-glossary.md`. Two of these (38992, 44663) were already named in the original 2026-09-15 finding, so the wording-only fix has not closed the gap in the time since. Worth treating this as needing something more structural than instruction wording — e.g. a required end-of-session check (now added to `prompts/requirements-intake.md`'s new "Final recap" step going forward) plus a deliberate decision on whether these 5 accounts' missing history is worth reconstructing retroactively or left as a known gap.
