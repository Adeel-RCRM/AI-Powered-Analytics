...
"drilldown_cards"/per-card "drill" keys via scripts/dashboard_drilldowns.py
(shared with create_default_dashboard.py): most cards get a single
click_behavior into a shared per-entity detail list; "Job's Breakdown By
Owner" (a multi-metric table) gets a per-column click_behavior instead, one
per named count-where metric, each into its own detail card whose filter
condition is copied verbatim from that metric's own aggregation. "Job's
Opening Duration" gets no drill-down at all - its query has no aggregation
clause, so it's already the most granular view. The native-SQL card's
drill-down targets the plain "assignments" entity detail list filtered by
`hiring_stage` (not a reimplementation of its LEAD() window function) and
carries no Date parameter passthrough, since a native-SQL card's `param_mapping`
is a template-tag reference with no equivalent plain field ref to reuse on a
plain-MBQL destination. All drill-down cards land in this account's
"Important Metrics Dashboard Drill-downs" sub-collection under Drill-downs -
never a pre-existing card/dashboard, per hard constraint 7.

Usage:
    python3 scripts/create_important_metrics_dashboard.py --profile <mb-profile> [--account <number>] [--test-collection-id <id>]
...
def main():
    ...
    for key, c in created_cards.items():
        ...
        pm = c["param_mapping"]
        if mapping and pm.get("kind") == "field":
            # a template_tag param_mapping (the native-SQL card) has no
            # equivalent plain field ref to reuse as a click_behavior target
            # on a plain-MBQL destination - that card's drill-down gets only
            # its hiring_stage column source, no Date passthrough (see
            # create_important_metrics_dashboard.py's own docstring).
            new_fid = resolved[pm["entity"]]["fields"].get(pm["column"])
            if new_fid is not None:
                param_sources.append((pm["parameter_slug"], param_names.get(pm["parameter_slug"], pm["parameter_slug"]),
                                       dashboard_drilldowns.field_ref(new_fid)))
        ...
        if drill and drill["kind"] in ("single", "per_column"):
            ...
            col_names = dashboard_drilldowns.last_stage_breakout_columns(
                orig_card["dataset_query"], lambda oid: (field_entity_map.get(str(oid)) or {}).get("column"))
            for cn in col_names:
    ...
    print(f"  {drilldowns_wired}/{len(created_cards)} dashcards wired with a drill-down "
          ...
          f"in '{DRILLDOWNS_SUBCOLLECTION_NAME}' / 'Drill-downs')")
    if skipped:
        print("  Skipped:")
    ...
    log_event(
        ...
        drilldowns_created=len(entity_detail_cards) + len(extra_drilldown_cards),
